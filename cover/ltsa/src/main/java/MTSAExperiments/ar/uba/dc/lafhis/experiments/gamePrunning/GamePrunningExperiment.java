...
public class GamePrunningExperiment extends Experiment {
	...
	public Dictionary<String, JSONCompatible> primitiveRunExperiment(Dictionary<String, JSONCompatible> parameters) throws NullArgumentException{
		...
		GRControllerGoal<String> grControllerGoal = ExperimentLTSHelper
		...
		env = new LTSAdapter<Long, String>(ControllerUtils.embedFluents(envMTS, grControllerGoal, new StandardOutput()),
		...
		LTS<Long, String> envForController = new LTSAdapter<Long, String>(ControllerUtils.embedFluents(envForCtrlMTS, grControllerGoal, new StandardOutput()),
				TransitionType.POSSIBLE);
		...
		StateSpaceCuttingPerfectInfoOppositeGrControlProblem grControlProblem = new StateSpaceCuttingPerfectInfoOppositeGrControlProblem(
				env, grControllerGoal, trapState, runNoG.getValue(), runAssumptions.getValue(), runGR.getValue(), allControllables.getValue(), 
		...
		results.put(ENV_GRAPH_RESULT, new JSONCompatibleObject(
				ExperimentJUNGHelper.getInstance().getLTSGraph(env, grControllerGoal.getControllableActions())
				));		
		...
		results.put(CTRL_GRAPH_RESULT, new JSONCompatibleObject(
				ExperimentJUNGHelper.getInstance().getLTSGraph(controller, grControllerGoal.getControllableActions())
				));
