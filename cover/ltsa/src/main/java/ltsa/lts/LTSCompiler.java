...
//			this.hideMaybeActions(p.alphaHidden);
	...
	private void updateControllerDefinition(UpdatingControllersDefinition ucDefinition) {
		...
		while (current.kind == Symbol.MAPPING) {
			ucDefinition.addMap(this.getMap());
            next_symbol();
		}
    ...
    */
	...
	public Pair<List<Symbol>,List<Symbol>> getMap() {
		expectBecomes();
        List<Symbol> left = new ArrayList<>();
		expectLeftCurly();
		next_symbol();
		while (current.kind == Symbol.UPPERIDENT) {
			left.add(current);
		...
		Pair<List<Symbol>, List<Symbol>> definition = new Pair(left,right);
		return definition;
	}
}