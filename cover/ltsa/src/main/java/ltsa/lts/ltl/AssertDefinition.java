...
public class AssertDefinition {
	...
	private AssertDefinition(Symbol n, FormulaSyntax f, LabelSet ls,
			Hashtable ip, Vector p, boolean isConstraint, boolean b) {
		name = n;
		ltl_formula = f;
		cached = null;
		alphaExtension = ls;
	...
	public static void put(Symbol n, FormulaSyntax f, LabelSet ls,
		...
		if (definitions == null)
			definitions = new Hashtable<String, AssertDefinition>();
		if (constraints == null)
			constraints = new Hashtable<String, AssertDefinition>();
		if (!isConstraint) {
	...
	private static void compileAll(Hashtable definitions, LTSOutput output) {
		...
		while (e.hasMoreElements()) {
			...
			p.fac.setFormula(p.ltl_formula.expand(p.fac, new Hashtable(),
					p.init_params));
		}
	}
	...
	public static CompactState compileConstraint(LTSOutput output, Symbol name,
		...
		if (constraints == null)
			return null;
		AssertDefinition p = (AssertDefinition) constraints
				.get(name.toString());
		if (p == null)
			return null;
		p.cached = null;
		...
		if (!cs.isProperty && !cs.name.startsWith(AssertDefinition.NOT_DEF)) {
			Diagnostics.fatal("LTL constraint must be safety: " + p.name,
					p.name);
		}
		if (!p.isProperty) {
	...
	private static CompositeState compile(Hashtable<String, AssertDefinition> definitions,
			LTSOutput output, String asserted, boolean isconstraint) {
		if (definitions == null || asserted == null){
			return null;
		}
		Preconditions.checkArgument(definitions.containsKey(asserted), "No definition present for "+asserted);
		
		AssertDefinition p = definitions.get(asserted);
		
		Preconditions.checkNotNull(p.name, "The name of the formula associated with "+asserted+" is null ");
		Preconditions.checkNotNull(p.fac, "The factory of the formula associated with "+asserted+" is null ");
		Preconditions.checkNotNull(p.fac.getFormula(), "The factory of the formula associated with "+asserted+" is null ");
		...
		output.outln("Formula !" + p.name.toString() + " = "
				+ p.fac.getFormula());
		...
		// output.outln("SSR " + g.getNodeCount() + " states " +
		// g.getEdgeCount() + " transitions");
		Graph g1 = Degeneralize.degeneralize(g);
		// output.outln("DEG " + g1.getNodeCount() + " states " +
		// g1.getEdgeCount() + " transitions");
		g1 = SCCReduction.reduce(g1);
		// output.outln("SCC " + g1.getNodeCount() + " states " +
		// g1.getEdgeCount() + " transitions");
		g1 = Simplify.simplify(g1);
		g1 = SFSReduction.reduce(g1);
		// output.outln("SFS " + g1.getNodeCount() + " states " +
		// g1.getEdgeCount() + " transitions");
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		Converter c = new Converter(p.name.toString(), g1,
				gba.getLabelFactory());
