...
public class HPWindow extends JFrame implements LTSManager, LTSInput,
	...
	public JMenuItem build_compile;
	...
	public HPWindow(AppletButton isap) {
		...
		try {
			...
			// FileSystemXmlApplicationContext("src/main/java/ltsa/ltsa-context.xml");
		}
		...
		tools.add(newFileTool = createTool(getClass().getClassLoader()
				.getResource("icon/new.gif"), "New file", new NewFileAction()));
		tools.add(openFileTool = createTool(getClass().getClassLoader()
				.getResource("icon/open.gif"), "Open file",
				new OpenFileAction()));
		tools.add(saveFileTool = createTool(getClass().getClassLoader()
				.getResource("icon/save.gif"), "Save File",
				new SaveFileAction()));
		tools.addSeparator();
		tools.add(cutTool = createTool(
				getClass().getClassLoader().getResource("icon/cut.gif"), "Cut",
				new EditCutAction()));
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/copy.gif"),
				"Copy", new EditCopyAction()));
		tools.add(pasteTool = createTool(getClass().getClassLoader()
				.getResource("icon/paste.gif"), "Paste", new EditPasteAction()));
		tools.add(undoTool = createTool(getClass().getClassLoader()
				.getResource("icon/undo.gif"), "Undo", new UndoAction()));
		tools.add(redoTool = createTool(getClass().getClassLoader()
				.getResource("icon/redo.gif"), "Redo", new RedoAction()));
		tools.addSeparator();
		tools.add(parseTool = createTool(getClass().getClassLoader()
				.getResource("icon/parse.gif"), "Parse", new DoAction(DO_parse)));
		tools.add(compileTool = createTool(getClass().getClassLoader()
				.getResource("icon/compile.gif"), "Compile", new DoAction(
				DO_compile)));
		tools.add(composeTool = createTool(getClass().getClassLoader()
				.getResource("icon/compose.gif"), "Compose", new DoAction(
				DO_doComposition)));
		tools.add(minimizeTool = createTool(getClass().getClassLoader()
				.getResource("icon/minimize.gif"), "Minimize", new DoAction(
				DO_minimiseComposition)));
		...
		tools.addSeparator();
		tools.add(safetyTool = createTool(getClass().getClassLoader()
				.getResource("icon/safety.gif"), "Check safety", new DoAction(
				DO_safety)));
		tools.add(progressTool = createTool(getClass().getClassLoader()
				.getResource("icon/progress.gif"), "Check Progress",
		...
		tools.addSeparator();
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/alphabet.gif"),
				"Run Arranged Animation", new DoAction(DO_ARRANGED_ANIMATOR)));
		tools.addSeparator();
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/exploration.gif"),
				"Do Exploration", new DoAction(DO_EXPLORATION)));
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/manual.png"),
				"Manual", new DoAction(DO_EXPLORATION_MANUAL)));
		tools.add(stepscount = createTextBox());
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/stepover.png"),
				"Step Over", new DoAction(DO_EXPLORATION_STEPOVER)));
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/resume.png"),
				"Resume", new DoAction(DO_EXPLORATION_RESUME)));

		getContentPane().add("North", tools);
		tools.addSeparator();
		tools.add(createTool(
				getClass().getClassLoader().getResource("icon/blanker.gif"),
	...
	protected JButton createTool(URL url, String tip, ActionListener act) {
	...
	public void doOpenFile(String dir, String f, boolean resource) {
		if (f != null)
			try {
				openFile = f;
	...
	class LayoutOptionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			// final JSpinner tree_distx_spinner = new JSpinner(new
			// SpinnerNumberModel(LTSJUNGCanvas.Tree_distX,0,1000,1));
			// treepanel.add(tree_distx_spinner);
			//
			// treepanel.add(new JLabel("Vertical distance"));
			// final JSpinner tree_disty_spinner = new JSpinner(new
			// SpinnerNumberModel(LTSJUNGCanvas.Tree_distY,0,1000,1));
			...
			// final JSpinner radial_distx_spinner = new JSpinner(new
			// SpinnerNumberModel(LTSJUNGCanvas.Radial_distX,0,1000,1));
			// radialpanel.add(radial_distx_spinner);
			//
			// radialpanel.add(new JLabel("Vertical distance"));
			// final JSpinner radial_disty_spinner = new JSpinner(new
			// SpinnerNumberModel(LTSJUNGCanvas.Radial_distY,0,1000,1));
			...
			class ButtonOptionListener implements ActionListener {
				...
				private void updateValues() {
					...
					// if (o==null) return;
					// LTSJUNGCanvas.Tree_distX = Integer.parseInt(o.toString())
					// < 0 ? 0 : (Integer.parseInt(o.toString()) > 1000 ? 1000 :
					...
					// if (o==null) return;
					// LTSJUNGCanvas.Tree_distY = Integer.parseInt(o.toString())
					// < 0 ? 0 : (Integer.parseInt(o.toString()) > 1000 ? 1000 :
					// Integer.parseInt(o.toString()));
					// //if (layouts!=null)
					// layouts.setFR_max_iterations(LTSLayoutWindow.FR_max_iterations);
					...
					// if (o==null) return;
					// LTSJUNGCanvas.Radial_distX =
					// Integer.parseInt(o.toString()) < 0 ? 0 :
					// (Integer.parseInt(o.toString()) > 1000 ? 1000 :
					// Integer.parseInt(o.toString()));
					// //if (layouts!=null)
					// layouts.setFR_max_iterations(LTSLayoutWindow.FR_max_iterations);
					// } catch(NumberFormatException nfe) {}
	...
	private boolean compile(LTSOutput output) {
		...
		postState(current);
		int[] current_states = new int[current.machines.size()];
	...
	public void coverLiveness(LTSOutput output) {
		clearOutput();
		CompositeState ltl_property = AssertDefinition.compile(this, asserted);
		// Silent compilation for negated formula
		CompositeState not_ltl_property = AssertDefinition.compile(
		...
		if (current != null && ltl_property != null) {
			TransitionSystemDispatcher.checkFLTL(current, ltl_property,
					not_ltl_property, this.setFair.isSelected(), output);
			postState(current);
		}
	}

	public void liveness(LTSOutput output) {
