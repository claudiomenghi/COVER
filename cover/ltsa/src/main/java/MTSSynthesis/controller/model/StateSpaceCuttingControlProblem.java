...
public abstract class StateSpaceCuttingControlProblem<S, A> extends
	...
	public GRControllerGoal<A> getGRControllerGoal(){
		return grControllerGoal;
	}
	
	public GRGoal<S> getGRGoal(){
		return getGRGoalFromControllerGoal(environment, grControllerGoal);
	}
	...
	public StateSpaceCuttingControlProblem(LTS<S, A> env,
			GRControllerGoal<A> grControlGoal, S trapState) {
	...
	protected GRGoal<S> getGRGoalFromControllerGoal(LTS<S,A> environment, GRControllerGoal<A> grControllerGoal){
		GRGameBuilder<S, A> grGameBuilder = new GRGameBuilder<S,A>();
		return grGameBuilder.buildGRGameFrom(new MTSAdapter<S,A>(environment), grControllerGoal).getGoal();
	}	
